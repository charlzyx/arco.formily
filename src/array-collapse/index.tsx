import {
  ...
  Card,
  Collapse,
  CollapseItemProps as CollapsePanelProps,
  CollapseProps,
  Empty,
...
const takeDefaultActiveKeys = (
  ...
  return range(defaultOpenPanelCount).map((x) => x.toString());
};

const insertActiveKeys = (activeKeys: string[], index: number) => {
  if (activeKeys.length <= index) return activeKeys.concat(index.toString());
  return activeKeys.reduce((buf, key) => {
    if (+key < index) return buf.concat(key);
    if (+key === index)
      return buf.concat([key.toString(), (key + 1).toString()]);
