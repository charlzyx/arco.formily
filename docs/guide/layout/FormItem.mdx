...
> 全新的 FormItem 组件，相比于 Arco 的 FormItem，它支持的功能更多，同时它的定位是纯样式组件，不管理表单状态，所以也会更轻量，更方便定制
...
```tsx
import React from "react";
import { Input, FormItem, FormButtonGroup, Submit } from "formily-arco-react";
import { createForm } from "@formily/core";
import { FormProvider, createSchemaField } from "@formily/react";
  ...
  return (
    <FormProvider form={form}>
      <SchemaField>
        ...
        <SchemaField.Void
          x-component="Title"
          x-component-props={{ text: "是否撑满:暂未支持" }}
        />
...
import { Input, FormItem, FormLayout } from "formily-arco-react";
...
export default () => {
  ...
  const requiredSymbol = React.useMemo(() => {
    return typeof requiredMark === 'boolean' ? requiredMark: {position: requiredMark}
  }, [requiredMark])
  return (
    <ConfigProvider >
      <div>
      ...
      <br />
      <FormProvider form={form}>
        <FormLayout requiredSymbol={requiredSymbol}>
          <SchemaField>
            <SchemaField.String
              title="我是必填项"
              required
...
import { FormProvider, createSchemaField } from "@formily/react";
import {
  IconCheckCircle as CheckCircleFilled,
  IconLoading as LoadingOutlined,
  ...
  return (
    <FormProvider form={form}>
      <SchemaField>
        ...
        <SchemaField.String
          title="加载状态(feedbackStatus=validating)"
          x-decorator="FormItem"
          x-component="Input"
          description="description"
          x-decorator-props={{
...
const form = createForm({
  ...
  effects: () => {
    onFieldChange("size", ["value"], (field, form) => {
      const layout = form.query('sizeWrap').take();
      if (!layout) return;
      console.log('layout', layout)
      layout.componentProps.size = field.value;
...
export default () => {
  return (
    <FormProvider form={form}>
      <SchemaField>
        <SchemaField.Void name="sizeWrap" x-component="FormLayout">
          <SchemaField.String
            name="size"
            title="Radio.Group"
            x-decorator="FormItem"
